"""SHAP explanation script for the GRU classifier.

Selects the best GRU run by test F1, reloads its checkpoint, computes SHAP
...
from gru.train_gru import GRUClassifier
...
# Metric column from all_results_gru.xlsx used to rank and select the best run.
RANK_BY: str = "test_f1"
...
def _build_model(hp: dict, input_size: int, num_classes: int) -> GRUClassifier:
...
if __name__ == "__main__":
    ...
    all_results_path = results_dir / "all_results_gru.xlsx"
    out_dir = results_dir / "SHAP_GRU"
    ...
    run_dir, metrics = find_best_run(
        all_results_path, results_dir, "results_gru_", RANK_BY
    )
    ...
    # Feature values averaged over time steps, used to colour the summary plot.
