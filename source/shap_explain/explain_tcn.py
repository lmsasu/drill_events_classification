"""SHAP explanation script for the TCN classifier.

Selects the best TCN run by test F1, reloads its checkpoint, computes SHAP
values on the test set, and writes plots + a Markdown report to
``results/SHAP_TCN/``.

Note: TCNClassifier.forward() accepts ``(batch, seq_len, features)`` and
transposes internally, so the SHAP interface is identical to GRU/LSTM.
"""
...
from tcn.train_tcn import TCNClassifier
...
from shap_explain.shap_core import (
    EXPLAINER_CHOICES,
    aggregate_shap,
    build_explainer,
    compute_shap,
...
# "deep" and "gradient" are GPU-friendly and fast; "kernel" is model-agnostic but slow.
...
# Metric column from all_results_tcn.xlsx used to rank and select the best run.
RANK_BY: str = "test_f1"
...
def _build_model(hp: dict, input_size: int, num_classes: int) -> TCNClassifier:
    return TCNClassifier(
        input_size=input_size,
        hidden_size=int(hp["hidden_size"]),
        num_levels=int(hp["num_levels"]),
...
if __name__ == "__main__":
    ...
    all_results_path = results_dir / "all_results_tcn.xlsx"
    out_dir = results_dir / "SHAP_TCN"
    out_dir.mkdir(parents=True, exist_ok=True)

    dataset_path = repo_root / "data" / "DataBaseTCN.xlsx"
    df_raw = load_dataset(dataset_path)
    feature_names = [c for c in df_raw.columns if c != "Task"]

    run_dir, metrics = find_best_run(all_results_path, results_dir, "results_tcn_", RANK_BY)
    ...
    logger.info(
        ...
        EXPLAINER_TYPE, len(background), len(x_explain),
    )
    explainer = build_explainer(EXPLAINER_TYPE, model, background)
    sv = compute_shap(explainer, x_explain, EXPLAINER_TYPE)
    logger.info("SHAP array shape (n_classes, n_samples, seq_len, n_features): %s", sv.shape)
