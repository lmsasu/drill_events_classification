...
def play(
    ...
    **hyperparameters: Any,
    ...
    y_true_train, y_pred_train, y_probs_train = _collect_predictions(
        model, train_loader, device
    )
    y_true_val, y_pred_val, y_probs_val = _collect_predictions(
        model, val_loader, device
    )
    y_true_test, y_pred_test, y_probs_test = _collect_predictions(
    ...
    with open(os.path.join(dir_name, "classification_report.txt"), "wt") as f:
        for split, loss, metrics, y_true, y_pred in [
            ...
            report = classification_report(
                y_true, y_pred, target_names=original_labels, output_dict=False
            )
