"""
K-Nearest Neighbours activity classifier for the day-tractor dataset.
...
def _align_proba(proba: np.ndarray, model_classes: np.ndarray, n_classes: int) -> np.ndarray:
    ...
    ``KNeighborsClassifier.predict_proba`` only includes columns for classes
...
def play(
    ...
    **hyperparameters: Any,
    ...
    dir_name  = str(RESULTS_DIR / f"results_knn_{timestamp}")
