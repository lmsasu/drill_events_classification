"""
Decision Tree activity classifier for the day-tractor dataset.
...
def _align_proba(
    ...
    ``DecisionTreeClassifier.predict_proba`` only includes columns for classes
...
def play(
    ...
    **hyperparameters: Any,
    ...
    """Fit and evaluate a Decision Tree on the day-tractor splits.
    ...
    Args:
        ...
        **hyperparameters: Training configuration.  Expected keys:
            ``window_size``, ``max_depth``, ``criterion``, ``max_features``,
    ...
    dir_name = str(RESULTS_DIR / f"results_dt_{timestamp}")
    ...
    logger.info(
        "Model fitted in %.3f seconds  (depth=%d, leaves=%d)",
        fit_time,
        model.get_depth(),
        model.get_n_leaves(),
    )
