"""
MLP activity classifier for the day-tractor dataset.
...
def _align_proba(proba: np.ndarray, model_classes: np.ndarray, n_classes: int) -> np.ndarray:
    ...
    ``MLPClassifier.predict_proba`` only includes columns for classes that
    appeared in the training set.  If a class had zero training windows its
    ...
    **hyperparameters: Any,
    ...
    Args:
        ...
        **hyperparameters: Training configuration.  Expected keys:
            ``window_size``, ``hidden_layer_sizes``, ``activation``,
    ...
    run_config.setdefault("early_stopping",    False)

    logger.info("Play called with hyperparameters: %s", run_config)

    timestamp = pd.Timestamp.now().strftime("%Y-%m-%d_%H-%M-%S")
    dir_name  = str(RESULTS_DIR / f"results_mlp_{timestamp}")
