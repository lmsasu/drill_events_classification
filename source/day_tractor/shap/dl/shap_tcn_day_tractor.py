"""
SHAP feature-importance analysis for the best TCN trial.

Explainer: shap.GradientExplainer (expected gradients, PyTorch-native).
           GradientExplainer is preferred over DeepExplainer here because
           TCN's forward() calls transpose(), which DeepExplainer may not
           trace correctly; GradientExplainer works through any differentiable op.
...
from day_tractor.dl.train_tcn_day_tractor import TCNClassifier
...
ALL_RESULTS    = RESULTS_DIR  / "all_results_tcn.xlsx"
N_BACKGROUND   = 100
N_TEST_SAMPLES = 300
...
def main() -> None:
    ...
    ckpt = RESULTS_DIR / f"results_tcn_{timestamp}" / "best_model.pt"
