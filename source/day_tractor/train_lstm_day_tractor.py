"""
LSTM activity classifier for the day-tractor dataset.
...
Key differences from the continuous-dataset LSTM trainer
---------------------------------------------------------
...
# ── paths ─────────────────────────────────────────────────────────────────────
DATA_DIR    = Path(__file__).resolve().parents[2] / "data" / "day_tractor"
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "day_tractor" / "LSTM"
...
def play(
    ...
    save_eps: bool = True,
    **hyperparameters: Any,
) -> tuple[dict[str, list[float]], LSTMClassifier]:
    """Train and evaluate the LSTM classifier on the day-tractor splits.
    ...
    dir_name  = str(RESULTS_DIR / f"results_lstm_{timestamp}")
    os.makedirs(dir_name, exist_ok=True)
    ...
    all_results_path = str(RESULTS_DIR / "all_results_lstm.xlsx")
