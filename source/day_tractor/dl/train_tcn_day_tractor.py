"""
TCN activity classifier for the day-tractor dataset.
...
Key differences from the continuous-dataset TCN trainer
---------------------------------------------------------
...
* TCN expects input of shape ``(batch, features, seq_len)``; the forward
  pass transposes from the loader's ``(batch, seq_len, features)`` format.
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[3] / "results" / "day_tractor" / "dl" / "TCN"
...
class TCNClassifier(nn.Module):
    ...
    The TCN processes the full input sequence; the output at the final time
    step is passed to a fully-connected layer that produces one logit per
    class.  The effective receptive field grows exponentially with depth:
    ``(kernel_size - 1) * (2 ** num_levels - 1) + 1``.

    Args:
        input_size: Number of input features at each time step.
        hidden_size: Number of channels in every TCN residual level.
        num_levels: Number of stacked dilated-convolution levels.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: Input tensor of shape ``(batch, window_size, input_size)``
               as returned by the DataLoader.

        Returns:
            Logits tensor of shape ``(batch, num_classes)``.
        """
        # TCN expects (batch, channels, seq_len); transpose from loader format.
...
def play(
    ...
    save_eps: bool = True,
    **hyperparameters: Any,
) -> tuple[dict[str, list[float]], TCNClassifier]:
    """Train and evaluate the TCN classifier on the day-tractor splits.
    ...
    dir_name  = str(RESULTS_DIR / f"results_tcn_{timestamp}")
    os.makedirs(dir_name, exist_ok=True)
    ...
    model = TCNClassifier(
        ...
        num_levels  = int(run_config["num_levels"]),
    ...
    all_results_path = str(RESULTS_DIR / "all_results_tcn.xlsx")
    ...
    row: dict[str, Any] = {"model": "TCN", "timestamp": timestamp}
